    ...
    <div class="card-body p-4">
        ...
        <div class="d-flex justify-content-end gap-2">
            <div class="btn-group" role="group">
                <a href="{{ route('shipments.print', ['shipment' => $shipment, 'size' => 'a4']) }}" class="btn btn-primary" target="_blank"><i class="fas fa-print me-1"></i> Cetak A4</a>
            ...
            <form action="{{ route('shipments.destroy', $shipment) }}" method="POST" class="d-inline">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"><i class="fas fa-trash-alt me-1"></i> Hapus</button>
            </form>
            <a href="{{ route('shipments.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left me-1"></i> Kembali</a>
        </div>
    </div>
</div>
@endsection